...
class Solution {
    ...
    public int maxConsecutiveAnswers(String answerKey, int k) {
		return Math.max(changeChar(answerKey, k, 'T'), changeChar(answerKey, k, 'F'));
    }
	public int changeChar(String answerKey, int k, char c){
		int len = 0, cnt = 0, left = 0, right = 0;
		for(right = 0; right < answerKey.length(); right++){
			if(answerKey.charAt(right) != c) cnt++;
			while(cnt > k){
