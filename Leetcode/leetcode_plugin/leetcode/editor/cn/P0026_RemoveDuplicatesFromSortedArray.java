...
class Solution {
    public int removeDuplicates(int[] nums) {
		//fast为当前比较的元素位置，slow为当前比较nums[fast] == nums[fast - 1]时要填入的位置。
		// 因为fast永远>=slow,所以无需考虑更改nums[slow]后会影响下一次判断。
		//所以原地删除并不需要真删除，只需要覆盖即可。
		int slow = 1, fast = 1, k = nums.length;
		if(k == 0) return 0;
		for(; fast < k; fast++){
